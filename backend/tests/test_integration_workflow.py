...
async def test_complete_dynamic_config_workflow(mock_chainlit_context, test_config: config.ChainlitConfig):
    ...
    async with mock_chainlit_context as context:
        ...
        async def handle_profile_switch(profile: ChatProfile):
            ...
            if profile.name == "text-assistant":
                await update_config({
                    "features": {
                        "spontaneous_file_upload": {"enabled": False}
                    },
                    "ui": {
                        "name": "Text Assistant",
        ...
        overrides = context.session.config_overrides
        assert overrides["features"]["spontaneous_file_upload"]["enabled"] is True
        assert overrides["features"]["spontaneous_file_upload"]["accept"] == ["application/pdf", "text/*"]
        ...
        assert profile_switch_history == ["vision-assistant", "research-assistant", "text-assistant"]
...
async def test_configuration_merging_and_isolation(mock_session_factory):
    ...
    assert session2.config_overrides["features"]["spontaneous_file_upload"]["enabled"] is False
...
def test_backward_compatibility(test_config: config.ChainlitConfig):
    ...
    assert test_config.code.on_profile_switch is not None  # New callback is also available