...
def test_socket_profile_change_functionality():
    ...
    async def mock_change_chat_profile(
        session, profile_name, set_chat_profiles_func, on_profile_switch_func
    ):
    ...
    async def run_test():
        result = await mock_change_chat_profile(
            mock_session, "vision-model", mock_set_profiles, mock_callback
        )
...
def test_profile_switching_edge_cases():
    ...
    async def test_unknown_profile():
        ...
        test_profiles = [
            ChatProfile(name="known-profile", markdown_description="Known")
        ]
        ...
        result = await mock_change_chat_profile(
            mock_session, "unknown-profile", mock_set_profiles, None
        )
    ...
    async def test_no_profiles():
        ...
        result = await mock_change_chat_profile(mock_session, "any-profile", None, None)
    ...
    async def test_exception():
        ...
        result = await mock_change_chat_profile(
            mock_session, "any-profile", failing_set_profiles, None
        )
