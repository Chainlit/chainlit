...
async def auto_connect_mcp_servers() -> None:
    ...
    servers_to_connect = [
        server for server in config.features.mcp.servers if server.auto_connect
    ]
    ...
    for i, result in enumerate(results):
        ...
        if isinstance(result, Exception):
            logger.error(
                f"Failed to auto-connect to MCP server '{server_name}': {result}"
            )
        else:
    ...
    logger.info(
        f"MCP auto-connection completed: {successful_connections}/{len(servers_to_connect)} successful"
    )
...
async def _connect_single_server(server_config: McpServerConfig) -> None:
    ...
    if server_config.name in session.mcp_sessions:
        logger.debug(
            f"MCP server '{server_config.name}' is already connected, skipping"
        )
        return
    ...
    try:
        ...
        raise RuntimeError(
            f"Connection timeout ({timeout}s) for server '{server_config.name}'"
        )
    except Exception as e:
        raise RuntimeError(
            f"Connection failed for server '{server_config.name}': {e!s}"
        )
