...
import { type IPdfjsElement } from 'client-types/';
...
interface Props {
  element: IPdfjsElement;
}

const PDFjsElement = ({ element }: Props) => {
  ...
  if (element.display === 'side' || element.display === 'page') {
    return (
      <PDFjsViewer
        ...
        className={`${element.display}-pdfjs w-full h-[80vh] flex-1`}
      />
    );
  }
  ...
  return (
    <>
      <div
        ...
        className="inline-pdfjs relative group cursor-pointer border border-border rounded-md overflow-hidden bg-muted/20 w-full max-w-xs h-auto min-h-[160px] flex items-center justify-center hover:border-primary/50 transition-all p-2"
